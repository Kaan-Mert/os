...
def simulate_oom_killer_scenario(log_callback=print):
    ...
    log_callback(f"[OS LOG] [BELLEK] Sistem belleği (512 MB) tamamen doldu. Boş alan: {mem_manager.free_memory} MB")
    ...
    if not success:
        ...
        sacrificial_p = max(processes, key=lambda p: p.priority)
        ...
        log_callback(f"[OS LOG] [KURTARMA] Bellek açıldı. Yeni boş alan: {mem_manager.free_memory} MB. Kritik süreç tekrar tahsis ediliyor.")
