...
class PerformancePlotter:
    ...
    def plot_scheduler_gantt(fcfs_procs, rr_procs, mlfq_procs, filename="gantt_chart.png"):
        ...
        # Matplotlib kullanarak CPU planlama algoritmaları için 3'lü Gantt Şeması oluşturuluyor
        ...
        def draw_gantt_on_axis(ax, procs, title):
            ...
            # PID değerine göre basit sıralama (Selection Sort) - Lambda sort kaldırıldı
            sorted_procs = list(procs)
            for i in range(len(sorted_procs)):
                min_idx = i
                for j in range(i + 1, len(sorted_procs)):
                    if sorted_procs[j].pid < sorted_procs[min_idx].pid:
                        min_idx = j
                sorted_procs[i], sorted_procs[min_idx] = sorted_procs[min_idx], sorted_procs[i]
    ...
    def plot_page_replacement(capacities, fifo_faults, lru_faults, filename="page_faults.png"):
        ...
        # FIFO vs LRU sayfa hatası sayılarını gösteren çizgi grafiği çiziliyor
    ...
    def plot_disk_scheduling(initial_head, requests, fcfs_path, sstf_path, filename="disk_scheduling.png"):
        ...
        # Disk kafa hareketlerini kıyaslayan çift panelli grafik çiziliyor
        ...
        # Adım numaralarını yazdırıyoruz
    ...
    def plot_ssd_hdd_comparison(hdd_fcfs_time, hdd_sstf_time, ssd_time, filename="ssd_hdd.png"):
        ...
        # HDD vs SSD erişim sürelerini gösteren sütun grafiği çiziliyor
        plt.figure(figsize=(9, 5))
